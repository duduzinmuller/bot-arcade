...
export default {
  ...
  async execute(interaction, bot) {
    ...
    const stats = await bot.xpSystem.getUserStats(targetUser.id, interaction.guild.id);
    
    await interaction.deferReply();
    
    try {
      const attachment = await this.createProfileCard(targetUser, stats, interaction.guild);
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: '⭐ XP Total', value: `${stats.xp.toLocaleString()}`, inline: true },
          { name: '📈 Progresso', value: `${stats.progress.toFixed(1)}%`, inline: true },
          { name: '💬 Mensagens', value: `${stats.totalMessages.toLocaleString()}`, inline: true },
          { name: '🎯 Próximo Nível', value: `${stats.xpForNextLevel - stats.xp} XP`, inline: true },
          { name: '📊 XP Atual', value: `${stats.xp - stats.xpForCurrentLevel}/${stats.xpForNextLevel - stats.xpForCurrentLevel}`, inline: true }
        )
  ...
  async executeMessage(message, targetUser, stats, bot) {
    try {
      const attachment = await this.createProfileCard(targetUser, stats, message.guild);
  ...
  async createProfileCard(user, stats, guild) {
    ...
    ctx.fillText(`${currentXP.toLocaleString()} / ${neededXP.toLocaleString()} XP`, barX + barWidth / 2, barY + 20);
    ...
    ctx.fillText(`Mensagens: ${stats.totalMessages.toLocaleString()}`, 200, 225);
    ctx.fillText(`Progresso: ${stats.progress.toFixed(1)}%`, 200, 250);

    // Next level info
    ctx.textAlign = 'right';
    const xpToNext = stats.xpForNextLevel - stats.xp;
    ctx.fillText(`${xpToNext.toLocaleString()} XP para o próximo nível`, 750, 200);
